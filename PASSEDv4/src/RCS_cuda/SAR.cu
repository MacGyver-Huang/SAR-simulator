...
string SearchSuitableRCSFile(const D1<double>& ThetaSeriesRCS, const D1<double>& ThetaSeries, const size_t ProcLevel,
							 const D1<string>& file_RCSData, const EF& Ef, const string POL, const long it, const string Method=""){
	// e.g. For total
	...
	// Find the nearest theta(look) angle in RCS
	...
	string StrMatchLook = StrTruncate(num2str(rad2deg(val)), 2);
	...
	for(size_t i=0;i<file_RCSData.GetNum();++i){
		...
		if( def_func::StrUppercase(tmp2[0]) == "EXP"){
			continue;
		}

		string StrLook   = tmp2[1];
		string StrPol    = tmp2[3];
		string StrMethod = "TOTAL";
		string StrLevel  = "Level0";
		if(tmp2.GetNum() > 4){
			string method = tmp2[4];
			if(method == "PO"){
				StrMethod="PO";
				if(tmp2.GetNum() > 5){
					StrLevel=tmp2[5];
		...
		if( StrLook == StrMatchLook && StrPol == (POL + Ef.TxPol()) && StrLevel == "Level"+num2str(ProcLevel) && StrMethod == Method){
			file_RCSDataDest = file_RCSData[i];
		}

//		cout<<"----------------------------------------"<<endl;
//		cout<<"StrLook         = "<<StrLook<<endl;
//		cout<<"StrPol          = "<<StrPol<<endl;
//		cout<<"StrLevel        = "<<StrLevel<<endl;
//		cout<<"StrMethod       = "<<StrMethod<<endl;
//
//		cout<<"StrMatchLook    = "<<StrMatchLook<<endl;
//		cout<<"(POL + Ef.TxPol()) = "<<(POL + Ef.TxPol())<<endl;
//		cout<<"Level"+num2str(ProcLevel)<<endl;
//		cout<<"Method = "<<Method<<endl;
//
//		cout<<"file_RCSData[i] = "<<file_RCSData[i]<<endl;
...
void GenerateSAR(const SAR& Sar, const SV<double>& sv, const def::ORB& Orb, const MeshDef& mesh,		// SAR system
				 ...
				 // Return or keyword
				 D1<double>& theta_sq, const bool ShowSummary, const string Method, const bool IsCOMPLEX_MESSAGE = false){
	//+----------------------------------------------------------------------------------------+
	//|                                                                                        |
	//|                                 SINGLE SAR IMAGE                                       |
	//|                                                                                        |
...
//			Src_tmp[i].i() *= (Wa / Sar.Nr());
			Src_tmp[i].r() *= (1.0 / Sar.Nr());
			Src_tmp[i].i() *= (1.0 / Sar.Nr());
		}
		IFFT(Src_tmp);
		fftshift(Src_tmp);
	...
	// Method = {"TOTAL","PO","PTD"}
	if(Method == "PO"){
		Suffix = StrTheta + "_" + StrPhi + "_PO_"  + POL + Ef.TxPol();
	}else if(Method == "PTD"){
		Suffix = StrTheta + "_" + StrPhi + "_PTD_" + POL + Ef.TxPol();
	}else{
		Suffix = StrTheta + "_" + StrPhi + "_"     + POL + Ef.TxPol();
	}
	if(Method == "TOTAL"){ Suffix = StrTheta + "_" + StrPhi + "_" + POL + Ef.TxPol() + ""; }
	if(Method == "PO"){    Suffix = StrTheta + "_" + StrPhi + "_" + POL + Ef.TxPol() + "_PO"; }
	if(Method == "PTD"){   Suffix = StrTheta + "_" + StrPhi + "_" + POL + Ef.TxPol() + "_PTD"; }
	...
	string file_Srd        = dir_out + FullName + "/" + Suffix + "_Srd.raw";
	string file_RCM        = dir_out + FullName + "/" + Suffix + "_RCM.raw";
	string file_Srcmc      = dir_out + FullName + "/" + Suffix + "_Srcmc.raw";
	string file_Srcmc_rd   = dir_out + FullName + "/" + Suffix + "_Srcmc_rd.raw";
	...
	if(ProcLevel >= 0){
		file_Src        = dir_out + FullName + "/" + Suffix + "_Level" + num2str(ProcLevel) + "_Src.raw";
		file_Srd        = dir_out + FullName + "/" + Suffix + "_Level" + num2str(ProcLevel) + "_Srd.raw";
		file_Srcmc_rd   = dir_out + FullName + "/" + Suffix + "_Level" + num2str(ProcLevel) + "_Srcmc_rd.raw";
		file_Srcmc      = dir_out + FullName + "/" + Suffix + "_Level" + num2str(ProcLevel) + "_Srcmc.raw";
...
//		//| Kaiser window             |
//		//+---------------------------+
//		double KAISER_BETA_RG = 3.4;
//		D1<double> wr = sar::find::KaiserWindow(Sar.Fr(), Sar.Nr(), KAISER_BETA_RG);
//		fftshift(wr);
//
//		//+---------------------------+
//		//| Matching                  |
//		//+---------------------------+
//		D1<CPLX<double> > rgtmp(Src.GetN());
//		for(long j=0;j<Sar.Na();++j){
//			rgtmp = Src.GetRow(j);
//			FFT(rgtmp);
//			for(long i=0;i<Sar.Nr();++i){
//				rgtmp[i] = rgtmp[i] * wr[i];
//			}
//			// FFT
//			IFFT(rgtmp);
//			// assign back
...
//		}
		...
		//TODO: Change Migration HERE
		//+------------------------------+
		//|   Range central shift        |
		//|   (by phase multiplication)  |
		//+------------------------------+
...
//		D2<CPLX<double> > Srd = sar::RCMC(Src, RCM, false);
		...
		D2<CPLX<double> > Srcmc_rd = Srd;
		D2<CPLX<double> > Srcmc = Srcmc_rd;
		IFFTY(Srcmc);

		// Write to file
		Srd.WriteBinary(file_Srd.c_str());
		Srcmc_rd.WriteBinary(file_Srcmc_rd.c_str());
		Srcmc.WriteBinary(file_Srcmc.c_str());
		...
		for(long i=0;i<Sar.Nr();++i){
			for(long j=0;j<Sar.Na();++j){
				aztmp[j] = Srcmc_rd[j][i] * wk[j] * ref_az[j];
			}
			// FFT
			IFFT(aztmp);
			// assign back
...
void Usage(const string name){
	...
	cout<<"|    Usage   |"<<endl;
	cout<<"+------------+"<<endl;
	cout<<"  "<<name<<" <file_CNF> <file_SV> <dir_RCS> <dir_out> <name>"<<endl;
	cout<<"             [-LV <Level_Index>] [-TX <Tx_Pol>] [-NA <Na>] [-AS <Angle_Squint>]"<<endl;
	cout<<"             [-h] [-CM] [[-PO] [-PTD]] [-NINFO] [-WM <mode>] [-NNSV]"<<endl;
}
...
void Help(){
	...
	cout<<"|  Required  |"<<endl;
	cout<<"+------------+"<<endl;
	cout<<"  file_CNF           [string] file of SAR echo Simulation Parameter file"<<endl;
	cout<<"  file_SV            [string] file of state vector"<<endl;
	cout<<"  dir_RCS            [string] SAR echo folder"<<endl;
	cout<<"  dir_out            [string] SAR image export folder"<<endl;
	...
	cout<<"  4. When -PO is used, the PO focusing is activated. When -PTD is used, the PTD focusing is activated."<<endl;
	cout<<"     When -PO and -PTD are present at the same time, the total EM method focusing is enabled."<<endl;
	cout<<"     When both -PO and -PTD are not present, the total EM method focusing is also started (for compatibility with older versions).."<<endl;
}
...
////	string root_dir      = "/Users/cychiang/Documents/code/CLionProjects/PASSEDv3_RDA_ORG2/TEST_AREA/";
//	string root_dir      = "/Users/cychiang/Documents/Research_Life/PASSEDv3_Results/AntennaPattern/";
//
////	string file_Cnf      = root_dir + "/UD04_modified/UD04_SAR.json";
////	string file_Cnf      = root_dir + "/UD04_modified/UD04_SAR2.json";
//	string file_Cnf      = root_dir + "/UD04_modified/UD04_SAR3_2.0.json";	// theta_az=2.0
////	string file_Cnf      = root_dir + "/UD04_modified/UD04_SAR3_1.0.json";	// theta_az=1.0
////	string file_Cnf      = root_dir + "/UD04_modified/UD04_SAR3_0.5.json";	// theta_az=0.5
////	string file_Cnf      = root_dir + "/UD04_modified/UD04_SAR3_0.3.json";	// theta_az=0.3
//	string file_SV       = root_dir + "/UD04_modified/UD04.stv";
//
////	string projName      = "Ball3/res_Ball3_AntPat_comp2_GPU_S5_ThetaAz2";
////	string projName      = "Ball3/res_Ball3_AntPat_comp2_CPU_S5_ThetaAz2_NoAzGain";
////	string projName      = "Ball3/res_Ball3_AntPat_org0_S5";
////	string projName      = "res_Ball3_AntPat_comp2_GPU_S5_ThetaAz2_EqualdRad";
////	string projName      = "res_Ball3_AntPat_comp2_GPU_S5_ThetaAz2_EqualdRad2";
////	string projName      = "res_Ball3_AntPat_comp2_GPU_S5_ThetaAz2_EqualdRad3";
////	string projName      = "res_Ball3_AntPat_org0_S5";
////	string projName      = "res_Ball1_AntPat_S5_ThetaAz2";
//
////	string projName      = "Ball3_comp/res_Ball3_AntPat_S6_ThetaAz0.3_NoAzGain";
////	string projName      = "Ball3_comp/res_Ball3_AntPat_S6_ThetaAz0.3";
////	string projName      = "Ball3_Spotlight/res_Ball3_Spotlight_S5_ThetaAz0.3";
////	string projName      = "Ball3_Spotlight/res_Ball3_Spotlight_S5_ThetaAz0.3_Ver2";
////	string projName      = "Ball3_Spotlight/res_Ball3_Spotlight_S5_ThetaAz0.3_Ver3";
////	string projName      = "Ball3_Spotlight/res_Ball3_Spotlight_S5_ThetaAz2.0";
////	string projName      = "Ball3_Spotlight/res_Ball3_Spotlight_S5_ThetaAz2.0_Rad4529";
////	string projName      = "Ball3_Spotlight/res_Ball3_Stripmap_S5_ThetaAz2.0";
//
////	string projName      = "Ball3_comp2/res_Ball3_Stripmap_S5_ThetaAz2.0";
////	string projName      = "Ball3_comp2/res_Ball3_ISAR_S5_ThetaAz2.0";
//	string projName      = "Ball3_comp2/res_Ball3_AntPat";
//
//
////	string dir_RCS       = root_dir + projName + "/RCS/";
////	string dir_out       = root_dir + projName + "/SAR/";
//	string dir_RCS       = root_dir + projName + "/RCS/";
//	string dir_out       = root_dir + projName + "/SAR/";
//
//
...
//	tar.
	...
	// Read SAR.conf
	io::read::SimPar(file_Cnf, Sar, Ef, mesh, Orb, tar);
	Sar.SWrg() = 20;
...
//	if(cmp.GetVal("-AS", tmp)){ Sar.theta_sqc() = str2num<double>(tmp); }	// Squint Angle
	...
	// Write RAW data?
//	int WriteMode = 0; // Only focused
	int WriteMode = 2; // Focused file + aux. files
	...
	if( IsPO && !IsPTD){ Method.push_back("PO"); }		// Only PO
	if(!IsPO &&  IsPTD){ Method.push_back("PTD"); }		// Only PTD
	if(!IsPO && !IsPTD) {								// PO + PTD + Total
		Method.push_back("PO");
		Method.push_back("PTD");
		Method.push_back("TOTAL");
	}
	if( IsPO &&  IsPTD){								// PO + PTD + Total
		Method.push_back("PO");
		Method.push_back("PTD");
	...
	D1<string> file_RCSMeta = dr.GetSuffix("mtl");
	D1<string> file_RCSData = dr.GetSuffix("dat");
	file_RCSMeta = dir_RCS + file_RCSMeta;
	printf("%s , %s \n",file_RCSMeta[0].c_str(),dir_RCS.c_str());
	...
	MultiAngle MulAng;
	// All temporary variable will be removed after this processing stage
	{
		// Read Meta
		ifstream fin;
		fin.open(file_RCSMeta[0].c_str(), ios_base::binary);
		//printf("%d",file_RCSMeta[0]);
		if(fin.fail()){
			cerr<<"ERROR::Can not find file -> "<<file_RCSMeta[0]<<endl;
			exit(EXIT_FAILURE);
		}
...
//			D1<string> str = StrSplit(line, '=');
//			if(str.GetNum() > 1){
////				if(str[0] == "Look angle (min) [deg] "){ MulAng.LookFrom() = deg2rad(str2num<double>(str[1])); }
////				if(str[0] == "Look angle (max) [deg] "){ MulAng.LookTo() = deg2rad(str2num<double>(str[1])); }
////				if(str[0] == "Look angle (avg. step) [deg] "){ MulAng.LookTo() = deg2rad(str2num<double>(str[1])); }
////				if(str[0] == "Target Aspect angle (min) [deg] "){ MulAng.AspFrom() = deg2rad(str2num<double>(str[1])); }
////				if(str[0] == "Target Aspect angle (max) [deg] "){ MulAng.AspTo() = deg2rad(str2num<double>(str[1])); }
////				if(str[0] == "Target Aspect angle (avg. step) [deg] "){ MulAng.AspStep() = deg2rad(str2num<double>(str[1])); }
//			}
//		}
//		fin.close();


		MulAng.IsMulLook() = false;
		MulAng.LookFrom() = Sar.theta_l_MB();
		MulAng.LookTo() = 0;
		MulAng.LookStep() = 0;

		MulAng.IsMulAsp() = false;
		MulAng.AspFrom() = tar.Asp();
		MulAng.AspTo() = 0;
		MulAng.AspStep() = 0;
		...
		//+----------------------+
		dim[0] = Sar.Nr();
		dim[1] = MulAng.GetPhiSeries().size();
		dim[2] = MulAng.GetThetaSeries().size();
		dim[3] = Ef.RxPol().length();
		ThetaSeriesRCS = vector2D1(MulAng.GetThetaSeries());
		PhiSeriesRCS   = vector2D1(MulAng.GetPhiSeries());
	...
	vector<string> files = SearchFileInPASSEDEchoFolder(dir_RCS, name, "dat");
	for(size_t i=0;i<files.size();++i){
		string tmp0 = files[i].substr(0, files[i].size()-4);
		D1<string> tmp = StrSplit(tmp0, '_');
		string strMethod, strLevel;
		int Level;

		if(tmp.GetNum() > 4) {
			if(tmp[4] == "PO" || tmp[4] == "PTD"){
				strMethod = tmp[4];
				strLevel = (tmp.GetNum() > 5)? tmp[5]:"Level0";
			}else{
				strMethod = "TOTAL";
				strLevel = tmp[4];
			}
			Level = str2num<int>( strLevel.substr(5,1) );
		}else{
			strMethod = "TOTAL";
			strLevel  = "Level0";
			Level     = 0;
		...
		// Assign
		if(strMethod == "PO"){    if(Level > maxLevelPO ){   maxLevelPO    = Level; } }
		if(strMethod == "PTD"){   if(Level > maxLevelPTD){   maxLevelPTD   = Level; } }
		if(strMethod == "TOTAL"){ if(Level > maxLevelTotal){ maxLevelTotal = Level; } }
	}

	cout<<"maxLevel(PO)    = "<<maxLevelPO<<endl;
	cout<<"maxLevel(PTD)   = "<<maxLevelPTD<<endl;
	cout<<"maxLevel(TOTAL) = "<<maxLevelTotal<<endl;
	...
	//+------------------+
	int maxLevel;
	for(size_t iMed=0;iMed<Method.size();++iMed){
		// Message
		cout<<"Processing EM Method = "<<Method[iMed]<<endl;
		if(Method[iMed] == "PO"){    maxLevel = maxLevelPO; }
		if(Method[iMed] == "PTD"){   maxLevel = maxLevelPTD; }
		if(Method[iMed] == "TOTAL"){ maxLevel = maxLevelTotal; }
		for(size_t iProcLevel=0;iProcLevel<maxLevel+1;++iProcLevel){
			//+------------------+
			//|   Polarization   |
			//+------------------+
			for(unsigned long ipo=0;ipo<Ef.RxPol().length();++ipo){			// Polarization
				...
				for(size_t it=0;it<ThetaSeries.GetNum();++it){		// Theta (Look) angle
					...
					//====================================================================================
					string file_RCSDataDest = SearchSuitableRCSFile(ThetaSeriesRCS, ThetaSeries, iProcLevel, file_RCSData, Ef, POL, it, Method[iMed]);
					...
					for(size_t ip=0;ip<MulAng.GetPhiSeries().size();++ip){	// Phi (aspect) angle
						...
						GenerateSAR(Sar, sv, Orb, mesh, ThetaSeriesRCS, PhiSeriesRCS, PhiSeries[ip], R0, freq, Wr, Psc, Rmin, Rc_min, file_RCSDataDest, iProcLevel,
									Ef, ThetaSeries[it], POL, dir_out, name, WriteMode, Vs, TER_SV, theta_sq, ShowSummary, Method[iMed], IsCOMPLEX_MESSAGE);
